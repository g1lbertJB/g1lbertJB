...
int jailbreak_device(const char *uuid)
{
     ...
     */
    {
        if (backup_mkdir(backup, "MediaDomain", "Media/Recordings", 0755, 501, 501,
                         4) != 0)
        {
            ERROR("Could not make folder\n");
        }

        if (backup_symlink(backup, "MediaDomain", "Media/Recordings/.haxx", "/var/mobile",
                           501, 501, 4) != 0)
        {
            ERROR("Failed to symlink /var/mobile!\n");
        }

        if (backup_mkdir(backup, "MediaDomain", "Media/Recordings/.haxx/DemoApp.app", 0755,
                         501, 501, 4) != 0)
        {
            ERROR("Could not make folder\n");
        }
...
#undef ADD_FILE_EXEC
    ...
    /* Make the folders */
    {
        ...
        if (backup_symlink(backup, "MediaDomain", "Media/Recordings/.haxx", "/var/db", 501,
                           501, 4) != 0)
        {
     ...
     */
    {
        ...
        if (backup_symlink(backup, "MediaDomain", "Media/Recordings/.haxx", "/", 501, 501,
                           4) != 0)
        {
            ERROR("Failed to symlink root!\n");
        }

        if (backup_mkdir(backup, "MediaDomain", "Media/Recordings/.haxx/var/unthreadedjb",
                         0755, 0, 0, 4) != 0)
        {
            ERROR("Could not make folder\n");
        }
        ...
        {
            if (backup_add_file_from_path(backup, "MediaDomain", "payload/Unthread.app/unthread@1x.png",
        ...
        {
            if (backup_add_file_from_path(backup, "MediaDomain", "payload/Unthread.app/unthread.png",
                                          "Media/Recordings/.haxx/var/unthreadedjb/unthread.png",
                                          0100644, 0, 0, 4) != 0)
            {
                ERROR("Could not add unthread.png");
